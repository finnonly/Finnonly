...
public class SqliteSourceGenerator : IIncrementalGenerator
{
    public void Initialize(IncrementalGeneratorInitializationContext context)
    {
        ...
        // 3.5 提取 assembly 级别的 SqliteConfig
        var sqliteConfig = compilation.Select(static (comp, _) => GetSqliteConfig(comp));

        // 4. 注册DbContext生成（带配置）
        var entitiesWithConfig = entities.Combine(sqliteConfig);
        context.RegisterSourceOutput(entitiesWithConfig, (ctx, pair) => GenerateDbContext(ctx, pair.Left, pair.Right));

        // 5. 注册迁移管理器生成
        context.RegisterSourceOutput(entities, GenerateMigrationManager);
    }

    private static SqliteConfig GetSqliteConfig(Compilation compilation)
    {
        var config = new SqliteConfig();
        var attr = compilation.Assembly.GetAttributes()
            .FirstOrDefault(a => a.AttributeClass?.Name == "SqliteConfigAttribute");
        if (attr == null) return config;
        ...
        foreach (var namedArg in attr.NamedArguments)
        {
            ...
            {
                case "JournalMode":
                    config.JournalMode = namedArg.Value.Value as string ?? config.JournalMode;
                    break;
                case "Synchronous":
                    config.Synchronous = namedArg.Value.Value as string ?? config.Synchronous;
                    break;
                case "CacheSize":
                    config.CacheSize = (int)(namedArg.Value.Value ?? config.CacheSize);
                    break;
                case "TempStore":
                    config.TempStore = namedArg.Value.Value as string ?? config.TempStore;
                    break;
                case "MmapSize":
                    config.MmapSize = (long)(namedArg.Value.Value ?? config.MmapSize);
                    break;
                case "BusyTimeout":
                    config.BusyTimeout = (int)(namedArg.Value.Value ?? config.BusyTimeout);
                    break;
                case "ForeignKeys":
                    config.ForeignKeys = (bool)(namedArg.Value.Value ?? config.ForeignKeys);
                    break;
                case "AutoVacuum":
                    config.AutoVacuum = namedArg.Value.Value as string ?? config.AutoVacuum;
                    break;
                case "WalAutoCheckpoint":
                    config.WalAutoCheckpoint = (int)(namedArg.Value.Value ?? config.WalAutoCheckpoint);
                    break;
                case "PageSize":
                    config.PageSize = (int)(namedArg.Value.Value ?? config.PageSize);
                    break;
                case "SecureDelete":
                    config.SecureDelete = (bool)(namedArg.Value.Value ?? config.SecureDelete);
                    break;
            }
        }
    ...
    private void GenerateDbContext(SourceProductionContext context, ImmutableArray<EntityInfo?> entities, SqliteConfig config)
    {
    ...
    private string BuildDbContextCode(List<EntityInfo> entities, SqliteConfig config)
    {
        ...
        sb.AppendLine("        // 应用 PRAGMA 配置");
        sb.Append($"        using (var pragmaCmd = new SqliteCommand(\"");
        sb.Append($"PRAGMA page_size={config.PageSize}; ");
        sb.Append($"PRAGMA journal_mode={config.JournalMode}; ");
        sb.Append($"PRAGMA synchronous={config.Synchronous}; ");
        sb.Append($"PRAGMA cache_size={config.CacheSize}; ");
        sb.Append($"PRAGMA temp_store={config.TempStore}; ");
        sb.Append($"PRAGMA mmap_size={config.MmapSize}; ");
        sb.Append($"PRAGMA busy_timeout={config.BusyTimeout}; ");
        sb.Append($"PRAGMA foreign_keys={(config.ForeignKeys ? "ON" : "OFF")}; ");
        sb.Append($"PRAGMA auto_vacuum={config.AutoVacuum}; ");
        sb.Append($"PRAGMA wal_autocheckpoint={config.WalAutoCheckpoint}; ");
        sb.AppendLine($"PRAGMA secure_delete={(config.SecureDelete ? "ON" : "OFF")};\", _connection))");
        sb.AppendLine("        {");
        sb.AppendLine("            pragmaCmd.ExecuteNonQuery();");
        sb.AppendLine("        }");
    ...
    private void GenerateEntityMigrationMethod(StringBuilder sb, EntityInfo entity)
    {
        ...
        sb.AppendLine("        var exists = (long)(await existsCmd.ExecuteScalarAsync() ?? 0L) > 0;");
        sb.AppendLine();
